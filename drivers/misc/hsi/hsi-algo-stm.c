...
int hsi_write_dma_mode(struct hsi_channel *ch)
{
	...
	if (ch->write_data.is_dma) {
		src_addr = (void *)ch->write_data.dma_addr;
	} else {
		src_addr =
		    (void *)dma_map_single(NULL, ch->write_data.data,
					   ch->write_data.count, DMA_TO_DEVICE);
		ch->write_data.dma_addr = (dma_addr_t) src_addr;
	}
	dst_addr =
	...
	if (stm_enable_dma(ch->dma_pipe_id)) {
		stm_dbg(DBG_ST.hsi,
			"hsi_write_dma_mode: stm_enable_dma failed.\n");
		return -1;
	}
...
int hsi_read_dma_mode(struct hsi_channel *ch)
{
	...
	if (ch->read_data.is_dma) {
		dst_addr = (void *)ch->read_data.dma_addr;
	} else {
		dst_addr =
		    (void *)dma_map_single(NULL, ch->read_data.data,
					   ch->read_data.count,
					   DMA_FROM_DEVICE);
		ch->read_data.dma_addr = (dma_addr_t) dst_addr;
	}
