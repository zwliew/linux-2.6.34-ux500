...
int fill_hsi_dma_info(struct platform_device *pd, struct hsi_channel *ch) {
	...
	if(pd->id)
	{
		...
		if(err!=0) {
			printk("\nstm_request_dma for RX failed for channel %d, with er = %d", ch->channel_number, err);
			return err;
		}
		#endif
		...
		stm_set_callback_handler(ch->dma_pipe_id,
					     hsi_dma_read_eot_handler, (void *)ch);
		#endif
	...
	else
	{
		...
		if(err!=0) {
			...
			printk("\nstm_request_dma failed for TX for channel %d, with err = %d", ch->channel_number, err);
			return err;
			}
		#endif
		...
		#if 1
		stm_set_callback_handler(ch->dma_pipe_id,
					     hsi_dma_write_eot_handler, (void *)ch);
		#endif
	}
	return 0;
}
