...
static void __exit hsi_close_all_ch(struct hsi_controller *hsi_ctrlr)
{
	...
	if (hsi_ctrlr->dev_type == 0x0) {
		for (i = 0; i < hsi_ctrlr->max_ch; i++) {
			stm_free_dma(hsi_ctrlr->hsi_tx_channels[i].
					 dma_pipe_id);
		...
		for (i = 0; i < hsi_ctrlr->max_ch; i++) {
			stm_free_dma(hsi_ctrlr->hsi_rx_channels[i].
					 dma_pipe_id);
...
int fill_hsi_dma_info(struct platform_device *pd, struct hsi_channel *ch)
{
	...
	if (pd->id) {
...
#if 1
		err = stm_request_dma(&(ch->dma_pipe_id), ch->hsi_dma_info);
		if (err != 0) {
			stm_dbg(DBG_ST.hsi, "\nstm_request_dma for RX\
				failed for channel %d, with er = %d",
				ch->channel_number, err);
			return err;
		}
#endif
		 ...
		 * free_irq will be called by dma layer from
		 * the context of free_dma()
		 */
#if 1
		stm_set_callback_handler(ch->dma_pipe_id,
					     hsi_dma_read_eot_handler,
					     (void *)ch);
#endif
	} else {
		...
		err = stm_request_dma(&(ch->dma_pipe_id), ch->hsi_dma_info);
		if (err != 0) {
			stm_free_dma(ch->dma_pipe_id);
			stm_dbg(DBG_ST.hsi,
				"\nstm_request_dma failed for TX for\
				channel %d, with err = %d",
				ch->channel_number, err);
			return err;
		}
#endif
