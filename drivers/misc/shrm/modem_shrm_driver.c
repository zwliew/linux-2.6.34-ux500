...
/**debug functionality*/
#define ISA_DEBUG 0
#define dbg_printk(format, arg...)	(ISA_DEBUG & 1) ? (printk(KERN_ALERT NAME ": " format , ## arg)) : ({do {} while (0);})
...
//        priv->stats.rx_dropped++;
    ...
    memcpy(skb_put(skb, sizeof(struct phonethdr)), (void *)(data + 1), sizeof(struct phonethdr));
    memcpy(skb_put(skb, len), data, len);

    /* Write metadata, and then pass to the receive level */

    skb->dev = kmalloc(sizeof(struct net_device), GFP_ATOMIC);
...
static void rx_common_l2msg_handler(unsigned char l2_header,void *msg,unsigned int length)
{
...
static void rx_audio_l2msg_handler(unsigned char l2_header,void *msg,unsigned int length)
{
	dbgprintk("rx_audio_l2msg_handler++ \n");
...
static int __init shm_initialise_irq(struct shrm_dev *shm_dev_data)
{
	...
	err = request_irq(shm_dev_data->ca_wake_irq,ca_wake_irq_handler,0,"ca_wake-up",shm_dev_data);
		if (err < 0) {
	...
	err = request_irq(shm_dev_data->ac_read_notif_0_irq,ac_read_notif_0_irq_handler,0,"ac_read_notif_0",shm_dev_data);
		if (err < 0) {
		printk("Unable to allocate ac_read_notif_0_irq interrupt line\n");
			err = -EBUSY;
		free_irq(shm_dev_data->ac_read_notif_0_irq,shm_dev_data);
		}

	err = request_irq(shm_dev_data->ac_read_notif_1_irq,ac_read_notif_1_irq_handler,0,"ac_read_notif_1",shm_dev_data);
		if (err < 0) {
		printk("Unable to allocate ac_read_notif_1_irq interrupt line\n");
			err = -EBUSY;
		free_irq(shm_dev_data->ac_read_notif_1_irq,shm_dev_data);
		}

	err = request_irq(shm_dev_data->ca_msg_pending_notif_0_irq,ca_msg_pending_notif_0_irq_handler,0,"ca_msg_pending_notif_0",shm_dev_data);
	if (err < 0) {
		printk("Unable to allocate ca_msg_pending_notif_0_irq interrupt line\n");
			err = -EBUSY;
		free_irq(shm_dev_data->ca_msg_pending_notif_0_irq,shm_dev_data);
	}

	err = request_irq(shm_dev_data->ca_msg_pending_notif_1_irq,ca_msg_pending_notif_1_irq_handler,0,"ca_msg_pending_notif_1",shm_dev_data);
	if (err < 0) {
		printk("Unable to allocate ca_msg_pending_notif_1_irq interrupt line\n");
		err = -EBUSY;
...
void add_msg_to_queue(struct t_message_queue* q, unsigned int size)
{
	...
	new_msg = (struct t_queue_element*)kmalloc(sizeof(struct t_queue_element), GFP_KERNEL|GFP_ATOMIC);

	if(new_msg == NULL)
	{
		printk(":memory overflow inside while(1)\n");
		while(1);
	}
	...
	if(list_empty(&q->msg_list))
	{
		...
		/*There can be 2 blocking calls 1 is read call and another select call*/
 ...
 * @q: message queue
 *
 * This function delets a message from the message list associated with message queue q and also
 * updates read ptr.
 * If the message list is empty, then, event is set to block the select and read calls
 * of the paricular queue.
 *
 * The message list is FIFO style and message is always added to tail and removed from
 * head.
 */
...
static int get_size_of_new_msg(struct t_message_queue* q)
{
	...
	dbg_printk("Message No :%d\n",new_msg->no);
	dbg_printk("Offset :%d Size :%d\n",q->writeptr, new_msg->size);
	dbg_printk("get_size_of_new_msg--\n");
	return new_msg->size;
}
...
static void isi_receive(void* p_data, u32 n_bytes)
{
	...
	unsigned char *buf, *temp /*= (unsigned char *)kmalloc(n_bytes, GFP_ATOMIC)*/;
	ssize_t len;
	buf = (unsigned char *)kmalloc(n_bytes, GFP_ATOMIC);

	temp = buf;
	/* len = isa_read(&buf, n_bytes); */
	...
	kfree(temp);
#else
	/** This flag is set after a socket is created by user-space and it is bound to an address */
 ...
 * @n_bytes: message size
 *
 * This function is a callback to indicate security message reception is complete.
 * It updates Writeptr of the Fifo
 */
static void security_receive(void* p_data, u32 n_bytes)
{
 ...
 */
static unsigned int isa_select(struct file *filp, struct poll_table_struct *wait)
{
				...
				size_t len, loff_t *ppos)
{
	...
	spin_lock_bh(&q->update_lock);
		...
		dbg_printk("Waiting for Data\n" );
			...
			if(wait_event_interruptible(q->wq_readable, atomic_read(&q->q_rp) == 1)) {
				printk("Error wait_event_interruptible \n" );
				return -ERESTARTSYS;
			}
		...
		psrc=(char *)buf;
			...
			/*Copy First Part of msg*/
		if(copy_to_user(psrc, (unsigned char*)(q->p_fifo_base+q->readptr), size)) {
						printk(KERN_ALERT NAME ":copy_to_user failed\n");
						return -EFAULT;
			}
		psrc+=size;
			/*Copy Second Part of msg at the top of fifo*/
		if(copy_to_user(psrc, (unsigned char*)(q->p_fifo_base),(msgsize-size))) {
				printk(KERN_ALERT NAME ":copy_to_user failed\n");
				return -EFAULT;
			}
		}
		...
		{
		if(copy_to_user(buf, (unsigned char*)(q->p_fifo_base+q->readptr), msgsize)) {
				printk(KERN_ALERT NAME ":copy_to_user failed\n");
				return -EFAULT;
			}
		}
	...
	else
	{
		...
		if((q->readptr+msgsize)>=q->size)
		{
			...
			/*Copy First Part of msg*/
            // *buf = (unsigned char*)(q->pFifoBase+q->readptr);
			memcpy(buf, (unsigned char*)(q->p_fifo_base+q->readptr),size);

			/*Copy Second Part of msg at the top of fifo*/
            memcpy(buf+size, (unsigned char*)(q->p_fifo_base),(msgsize - size));

		}
		else
		{
			dbg_printk("sampath:: Inside Loop Back\n");
            // buf = (unsigned char*)(q->pFifoBase+q->readptr);
			memcpy(buf, (unsigned char*)(q->p_fifo_base+q->readptr), msgsize);
			dbg_printk("sampath:: *buf = %p, q->readptr = %d\n", (void *)*buf, q->readptr);
...
#if (ISA_DEBUG & 1)
		int i;
		dbg_printk(KERN_ALERT NAME "\nSAMPATH: isa_read buf :len = %d\n", len);
		for( i = 0 ; i < len ; i++)
			printk("%02X ", ((unsigned char*)(q->pFifoBase+q->readptr))[i]);
		printk("\n\n");
#endif
	}
 ...
 * DLP_IOCTL_ALLOCATE_BUFFER - To allocate buffer for new uplink message. This ioctl is
 * called with required message size. It returns offset for the allocates space in the queue.
 * DLP_IOCTL_PUT_MESSAGE -  To indicate new uplink message available in queuq for
 * transmission. Message is copied from offset location returned by previous ioctl before
 * calling this ioctl.
 * DLP_IOCTL_GET_MESSAGE - To check if any downlink message is available in queue. It returns
 * offset for new message inside queue.
 * DLP_IOCTL_DEALLOCATE_BUFFER - To deallocate any buffer allocate for downlink message
 * once the message is copied. Message is copied from offset location returned by previous
 * ioctl before calling this ioctl.
 */
...
static int isa_close(struct inode *inode, struct file *filp)
{
	...
	if(m==ISI_MESSAGING)
	{
		printk("Closed ISI_MESSAGING Device\n");
	}
	else if(m==RPC_MESSAGING)
	{
		printk("Closed RPC_MESSAGING Device\n");
	}
	else if(m==AUDIO_MESSAGING)
	{
		printk("Closed AUDIO_MESSAGING Device\n");
	}
	else if(m==SECURITY_MESSAGING)
	{
		printk("Closed SECURITY_MESSAGING Device\n");
	}
	else
	{
...
static int isa_open(struct inode *inode, struct file *filp)
{
	...
	if((m!=ISI_MESSAGING) && (m!=RPC_MESSAGING)&& (m!=AUDIO_MESSAGING)&& (m!=SECURITY_MESSAGING)) {
		printk(KERN_ALERT NAME ":No such device present\n");
		mutex_unlock(&isa_lock);
        return -ENODEV;
	}
	...
	if(m==ISI_MESSAGING)
	{
		dbg_printk("Open ISI_MESSAGING Device\n");
	}
	else if(m==RPC_MESSAGING)
	{
		dbg_printk("Open RPC_MESSAGING Device\n");
	}
	else if(m==AUDIO_MESSAGING)
	{
		dbg_printk("Open AUDIO_MESSAGING Device\n");
	}
	else if(m==SECURITY_MESSAGING)
	{
		dbg_printk("Open SECURITY_MESSAGING Device\n");
	}
	else
	{
		dbg_printk(KERN_ALERT NAME ":No such device present\n");
...
void do_phonet_rcv_tasklet(unsigned long unused)
{
	...
	buf = (unsigned char *)kmalloc(MAX_RCV_LEN, GFP_ATOMIC);
	...
	for(;;)
	{
		...
		len = isa_read(NULL, buf, MAX_RCV_LEN, NULL);
 ...
 *
 * This function registers module as a character driver using register_chrdev_region()
 * or alloc_chrdev_region. It adds this driver to system using cdev_add() call.
 * Major number is dynamically allocated using alloc_chrdev_region() by default or left
 * to user to specify it during load time. For this variable major is used as module_param
...
static int isa_init(void)
{
	...
	p_isa_context = (struct t_isa_driver_context*)kzalloc(sizeof(struct t_isa_driver_context), GFP_KERNEL);
	...
	for(no_dev=0;no_dev<ISA_DEVICES;no_dev++) {
		...
		p_isadev = (struct t_isadev_context*)kzalloc(sizeof(struct t_isadev_context), GFP_KERNEL);
		if(p_isadev==NULL) {
			printk(KERN_ALERT NAME ":Failed to alloc memory\n");
			return -ENOMEM;
		}
		...
		if(retval<0) {
			printk(KERN_ALERT NAME ":create_queue dl_queue failed\n");
		...
		//register device call backs


	}

	dbg_printk(KERN_ALERT NAME ": Driver added\n");
...
static int __init shrm_probe(struct platform_device *pdev)
{
	...
	if (pdev == NULL)  {
		printk( "No device/platform_data found on shm device\n");
		return -ENODEV;
	}
	...
	if (shm_dev_data == NULL) {
		printk("Could not allocate memory for struct shm_dev\n");
		return -ENOMEM;
	}
	...
	if (!res) {
	printk("Unable to map Ca Wake up interrupt \n");
		err = -EBUSY;
		goto rollback_intr;
	}
	...
	if (!res) {
		printk("Unable to map APE_Read_notif_common IRQ base \n");
		err = -EBUSY;
		goto rollback_intr;
	}
	...
	if (!res) {
		printk("Unable to map APE_Read_notif_audio IRQ base \n");
		err = -EBUSY;
		goto rollback_intr;
	}
	...
	if (!res) {
		printk("Unable to map Cmt_msg_pending_notif_common IRQ base \n");
		err = -EBUSY;
		goto rollback_intr;
	}
	...
	if (!res) {
		printk("Unable to map Cmt_msg_pending_notif_audio IRQ base \n");
		err = -EBUSY;
		goto rollback_intr;
	}
	...
	if (!res) {
		printk("Could not get SHM IO memory information\n");
		err = -ENODEV;
		goto rollback_intr;
	}

	shm_dev_data->intr_base = (void __iomem *)ioremap_nocache(res->start,res->end - res->start + 1);
#if 1/*Needs to be removed this configuring interrupts in edge trigger*/
	p_edge=(unsigned int *)((void __iomem *)ioremap_nocache(0xA0411C10,32));
	p_edge=(p_edge+2);
	*p_edge=0xFFFFFFFF;
#endif
	...
	shm_dev_data->ape_common_fifo_base_phy=(unsigned int*)U8500_SHM_FIFO_APE_COMMON_BASE;
	shm_dev_data->ape_common_fifo_base = (void __iomem *)ioremap_nocache(U8500_SHM_FIFO_APE_COMMON_BASE,SHM_FIFO_0_SIZE);
	...
	shm_dev_data->cmt_common_fifo_base_phy=(unsigned int*)U8500_SHM_FIFO_CMT_COMMON_BASE;

	shm_dev_data->cmt_common_fifo_base = (void __iomem *)ioremap_nocache(U8500_SHM_FIFO_CMT_COMMON_BASE,SHM_FIFO_0_SIZE);
	...
	shm_dev_data->ape_audio_fifo_base_phy=(unsigned int*)U8500_SHM_FIFO_APE_AUDIO_BASE;
	shm_dev_data->ape_audio_fifo_base = (void __iomem *)ioremap_nocache(U8500_SHM_FIFO_APE_AUDIO_BASE,SHM_FIFO_1_SIZE);
	...
	shm_dev_data->cmt_audio_fifo_base_phy=(unsigned int*)U8500_SHM_FIFO_CMT_AUDIO_BASE;
	shm_dev_data->cmt_audio_fifo_base = (void __iomem *)ioremap_nocache(U8500_SHM_FIFO_CMT_AUDIO_BASE,SHM_FIFO_1_SIZE);
	...
	shm_dev_data->ac_common_shared_wptr = (void __iomem *)ioremap(SHM_ACFIFO_0_WRITE_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ac_common_shared_rptr = (void __iomem *)ioremap(SHM_ACFIFO_0_READ_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ca_common_shared_wptr = (void __iomem *)ioremap(SHM_CAFIFO_0_WRITE_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ca_common_shared_rptr = (void __iomem *)ioremap(SHM_CAFIFO_0_READ_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ac_audio_shared_wptr = (void __iomem *)ioremap(SHM_ACFIFO_1_WRITE_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ac_audio_shared_rptr = (void __iomem *)ioremap(SHM_ACFIFO_1_READ_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ca_audio_shared_wptr = (void __iomem *)ioremap(SHM_CAFIFO_1_WRITE_AMCU,SHM_PTR_SIZE);
	...
	shm_dev_data->ca_audio_shared_rptr = (void __iomem *)ioremap(SHM_CAFIFO_1_READ_AMCU,SHM_PTR_SIZE);
	...
	if(isa_init()!=0)
	{
		printk("Driver Initialization Error \n");
		err = -EBUSY;
	}
	/* install handlers and tasklets */
	if (shm_initialise_irq(shm_dev_data)) {
		printk("shm error in interrupt registration \n");
		goto rollback_irq;
	}
...
static int __init shrm_driver_init(void)
{
	...
	if (err < 0) {
		printk("SHM Platform DRIVER register FAILED: %d\n", err);
		return err;
	}
...
static void __exit shrm_driver_exit(void)
{
	...
	printk("SHM Platform DRIVER removed\n");
}

module_init(shrm_driver_init);
module_exit(shrm_driver_exit);

MODULE_AUTHOR(SHRM_DRIVER_AUTHOR);
MODULE_DESCRIPTION(SHRM_DRIVER_DESC);
MODULE_LICENSE("GPL");
