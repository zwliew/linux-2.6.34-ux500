	...
	enum mcde_display_power_mode power_mode)
{
	...
	if (!ret && ddev->port->type == MCDE_PORTTYPE_DSI) {
		/* STANDBY -> ON */
	...
	} else if (!ret && ddev->port->type == MCDE_PORTTYPE_DPI)
...
static enum mcde_display_power_mode mcde_display_get_power_mode_default(
	struct mcde_display_device *ddev)
{
...
static void mcde_display_get_video_mode_default(
...
static enum mcde_ovly_pix_fmt mcde_display_get_pixel_format_default(
	struct mcde_display_device *ddev)
{
...
static enum mcde_display_rotation mcde_display_get_rotation_default(
	struct mcde_display_device *ddev)
{
...
static bool mcde_display_get_synchronized_update_default(
	struct mcde_display_device *ddev)
{
...
static int mcde_display_on_first_update_default(
					struct mcde_display_device *ddev)
{
...
int mcde_display_dsi_dcs_write(struct mcde_display_device *ddev,
	u8 cmd, u8 *data, int len)
{
	return mcde_dsi_dcs_write(ddev->chnl_state, cmd, data, len);
}
EXPORT_SYMBOL(mcde_display_dsi_dcs_write);

int mcde_display_dsi_dcs_read(struct mcde_display_device *ddev,
	u8 cmd, u8 *data, int *len)
{
	return mcde_dsi_dcs_read(ddev->chnl_state, cmd, data, len);
}
EXPORT_SYMBOL(mcde_display_dsi_dcs_read);
