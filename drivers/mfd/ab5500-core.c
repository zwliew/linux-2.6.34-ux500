...
static struct ab5500_bank bankinfo[AB5500_NUM_BANKS] = {
...
/* These stubbed prcmu functions should be removed when prcmu is in place */
int prcmu_abb_read(u8 slave, u8 reg, u8 *value, u8 size)
{
#ifdef CONFIG_MACH_U5500_SIMULATOR
	/* Fake chip_id to test. Remove when prcmu in place */
	if (slave == 0x4A && reg == 0x20)
		*value = 0x20;
	else
		*value = 0;
	return 0;
...
int prcmu_config_abb_event_readout(u32 abb_events)
{
#ifdef CONFIG_MACH_U5500_SIMULATOR
	return 0;
#else
...
static inline void ab5500_setup_debugfs(struct ab5500 *ab)
{
	...
	ab5500_reg_file = debugfs_create_file("all-registers",
...
static int __init ab5500_probe(struct platform_device *pdev)
{
	...
	err = get_register_interruptible(ab, 0, AB5500_CHIP_ID, &ab->chip_id);
	if (err) {
