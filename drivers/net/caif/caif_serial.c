...
static char *ser_ttyname = "/dev/ttyS0";
module_param(ser_ttyname, charp, S_IRUGO);
MODULE_PARM_DESC(ser_ttyname, "TTY to open.");

//deprecated-functionality-below
static int hw_flow_ctrl = 1;
module_param(hw_flow_ctrl, bool, S_IRUGO);
MODULE_PARM_DESC(hw_flow_ctrl, "Enable / Disable Hardware flow control");

#ifdef CONFIG_CAIF_TTY_NOFCS
static int ser_use_fcs;
#else
//deprecated-functionality-above
static int ser_use_fcs = 1;
//deprecated-functionality-below
#endif /* CONFIG_CAIF_TTY_NOFCS */
//deprecated-functionality-above
...
//deprecated-functionality-below
static int start_ldisc(char *tty_name)
{
	struct file *f;
	mm_segment_t oldfs;
	struct termios tio;
	...
	if (tty_name[0] != '/')
		return 0;
	f = filp_open(tty_name, 0, 0);
	if (IS_ERR(f)) {
		pr_err("CAIF cannot open:%s\n", tty_name);
		ret = -EINVAL;
		goto error;
	}
	if (f == NULL || f->f_op == NULL || f->f_op->unlocked_ioctl == NULL) {
		pr_err("TTY cannot do IOCTL:%s\n", tty_name);
		ret = -ENOTTY;
		goto error;
	}
	...
	/* Install the ldisc, this triggers ldisc_open() */
