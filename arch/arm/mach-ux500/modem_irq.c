...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 30)
static void modem_cpu_work(struct work_struct *work)
{
	int real_irq;
	int virt_irq;
	struct modem_irq *mi = container_of(work, struct modem_irq, work);
...
static irqreturn_t modem_cpu_irq_handler(int irq, void *data)
{
	...
	pr_debug("modem_irq: IRQ %d received!\n", irq);
	schedule_work(&mi->work);
...
static int modem_irq_init(void)
{
	...
	mi = kzalloc(sizeof(struct modem_irq), GFP_KERNEL);
	if (!mi) {
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 30)
	INIT_WORK(&mi->work, modem_cpu_work);

	/* Allocate the modem IRQ */
	err = request_irq(IRQ_MODEM, modem_cpu_irq_handler,
			  IRQF_DISABLED, "ModemIRQ", mi);
#else
	err = request_threaded_irq(IRQ_MODEM, NULL, modem_cpu_irq_handler,
