...
/* +1 equals +1*sizeof(u32) */
#define MBOX_FIFO_DATA        0x000
#define MBOX_FIFO_ADD         0x001
#define MBOX_FIFO_REMOVE      0x002
#define MBOX_FIFO_THRES_FREE  0x003
#define MBOX_FIFO_THRES_OCCUP 0x004
#define MBOX_FIFO_STATUS      0x005
...
try_send:
	...
	if (((mbox->write_index + 1) % MBOX_BUF_SIZE) == mbox->read_index) {
		if (!block) {
			res = -ENOMEM;
			goto exit;
		}
...
#if defined(CONFIG_DEBUG_FS)
	/* TODO: Move this to debugfs instead */
