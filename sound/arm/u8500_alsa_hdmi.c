...
static int configure_msp_hdmi(int sampling_freq, int channel_count)
{
	...
	//else
	//msp_config.input_clock_freq = mspInClockFreq;

	//if (mspClockSel == DEFAULT)
	//      msp_config.srg_clock_sel = SRG_CLK_SEL_APB;
	//else
	...
	// msp_config.protocol_desc.rx_data_transfer_width = MSP_DATA_TRANSFER_WIDTH_WORD;
	//   msp_config.protocol_desc.tx_data_transfer_width = MSP_DATA_TRANSFER_WIDTH_WORD;
	...
	msp_config.protocol_desc.tx_clock_pol = 0;	//rising
	...
	msp_config.multichannel_config.tx_channel_0_enable = 0x0000005;	//Channel 1 and channel 3
