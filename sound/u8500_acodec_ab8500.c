						     ...
						     size_t bytes, int dma_flag)
{
	...
	message.txdata = data;
	message.dma_flag = dma_flag;
	message.inf_loopback_xfer = MSP_INF_TRANSFER_ENABLED;
...
/*#ifdef CONFIG_U8500_AB8500_CUT10*/
	...
	if (DISABLE == acodec_config->direct_rendering_mode) {
		...
		if (acodec_config->tdm8_ch_mode == ENABLE) {
			writel(0x20, ((char *)(IO_ADDRESS(U8500_MSP1_BASE) + 0x70)));	//MSP_IODELAY
		} else {
			writel(0x00, ((char *)(IO_ADDRESS(U8500_MSP1_BASE) + 0x70)));	//MSP_IODELAY
		}
