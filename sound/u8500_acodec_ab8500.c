...
static void u8500_digital_lpbk_tx_dma_start();
static void u8500_digital_lpbk_rx_dma_start();
#endif
...
char *codec_in_texts[NUMBER_INPUT_DEVICE] = {
	"CODEC_SRC_LINEIN", "CODEC_SRC_MICROPHONE_1A","CODEC_SRC_MICROPHONE_1B",
	"CODEC_SRC_MICROPHONE_2", "CODEC_SRC_D_MICROPHONE_1","CODEC_SRC_D_MICROPHONE_2",
	"CODEC_SRC_D_MICROPHONE_3","CODEC_SRC_D_MICROPHONE_4","CODEC_SRC_D_MICROPHONE_5",
	"CODEC_SRC_D_MICROPHONE_6"
};
...
char *direct_rendering_state_in_texts[NUMBER_DIRECT_RENDERING_STATE] = {"DISABLE", "ENABLE"};
char *pcm_rendering_state_in_texts[NUMBER_PCM_RENDERING_STATE] = {"DISABLE", "ENABLE","PENDING"};
...
typedef struct
{
	...
	dmach_t rx_dma_pipeId;
	dmach_t tx_dma_pipeId;
	volatile int rx_active;
	volatile int tx_active;
	t_dma_buffer buffer[DIGITAL_LPBK_MAX_BIFFERS];
	int data_size;
	int rx_index;
	int tx_index;
}t_digital_lpbk_cnxt;
...
static struct i2sdrv_data i2sdrv;


t_ab8500_codec_error u8500_acodec_open(int stream_id)
{
        struct i2s_device *i2s;
	i2s = i2sdrv.i2s;
...
t_ab8500_codec_error u8500_acodec_send_data(void *data, size_t bytes, int dma_flag)
{
	struct i2s_device *i2s_dev=i2sdrv.i2s;
	int bytes_transmit;
...
t_ab8500_codec_error u8500_acodec_receive_data(void *data, size_t bytes, int dma_flag)
{
	struct i2s_device *i2s_dev=i2sdrv.i2s;
	int bytes_receive;
	struct i2s_message message;
...
t_ab8500_codec_error u8500_acodec_close(t_acodec_disable flag)
{
	struct i2s_device *i2s_dev=i2sdrv.i2s;
	int status = 0;
	if(i2sdrv.flag)
...
t_ab8500_codec_error u8500_acodec_enable_audio_mode(struct acodec_configuration *acodec_config)
{
	struct i2s_device *i2s_dev=i2sdrv.i2s;
	...
	FUNC_ENTER();
	stm_dbg(DBG_ST.acodec,"  Entering in u8500_acodec_enable_audio_mode()\n");
	...
	if (g_codec_system_context.cur_user == NO_USER) {
		stm_error
		    ("Audiocodec not yet configured by any user\n");
	...
	printk("\n acodec_config->channels = %d \n",acodec_config->channels);
...
//	{
		...
		if(acodec_config->channels == 1)
		{
			msp_config.multichannel_config.tx_channel_0_enable = 0x0000001;
		}
		else
		{
			msp_config.multichannel_config.tx_channel_0_enable = 0x0000003;
		}
		...
		if(acodec_config->channels == 1)
		{
			msp_config.multichannel_config.rx_channel_0_enable = 0x0000001;
		}
		else
		{
			msp_config.multichannel_config.rx_channel_0_enable = 0x0000003;
		}
...
//	{
	...
	codec_error =
	    AB8500_CODEC_SetModeAndDirection(acodec_config->direction, codec_in_mode,
					 codec_out_mode);
	if (AB8500_CODEC_OK != codec_error) {
	...
	codec_error = AB8500_CODEC_SetMasterMode(AB8500_CODEC_MASTER_MODE_ENABLE);
	...
	if (AB8500_CODEC_DIRECTION_IN == acodec_config->direction
	    || AB8500_CODEC_DIRECTION_INOUT == acodec_config->direction) {

		u8500_acodec_allocate_ad_slot(AB8500_CODEC_SRC_D_MICROPHONE_1,TDM_8_CH_MODE);
		u8500_acodec_allocate_ad_slot(AB8500_CODEC_SRC_D_MICROPHONE_2,TDM_8_CH_MODE);
	...
	if (AB8500_CODEC_DIRECTION_OUT == acodec_config->direction
	    || AB8500_CODEC_DIRECTION_INOUT == acodec_config->direction) {
			u8500_acodec_allocate_da_slot(AB8500_CODEC_DEST_HEADSET,TDM_8_CH_MODE);
...
/*#if DRIVER_DEBUG > 0
	...
	stm_dbg(DBG_ST.acodec,"leaving in u8500_acodec_enable_audio_mode() \n");
...
*/
t_ab8500_codec_error u8500_acodec_set_output_volume(t_ab8500_codec_dest dest_device,int left_volume,int right_volume,t_acodec_user user)
{
	...
	g_codec_system_context.output_config[dest_device].left_volume = left_volume;
	g_codec_system_context.output_config[dest_device].right_volume = right_volume;
...
t_ab8500_codec_error u8500_acodec_get_output_volume(t_ab8500_codec_dest dest_device,int *p_left_volume,int *p_right_volume,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	*p_left_volume = g_codec_system_context.output_config[dest_device].left_volume;
	*p_right_volume = g_codec_system_context.output_config[dest_device].right_volume;
...
* @out_right_volume - volume for right speaker
*/
t_ab8500_codec_error u8500_acodec_set_input_volume(t_ab8500_codec_src src_device,int left_volume,int right_volume,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	g_codec_system_context.input_config[src_device].left_volume = left_volume;
	g_codec_system_context.input_config[src_device].right_volume = right_volume;
...
* @out_right_volume - volume for right speaker
*/
t_ab8500_codec_error u8500_acodec_get_input_volume(t_ab8500_codec_src src_device,int *p_left_volume,int *p_right_volume,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	*p_left_volume = g_codec_system_context.input_config[src_device].left_volume;
	*p_right_volume = g_codec_system_context.input_config[src_device].right_volume;
...
* @out_right_volume - volume for right speaker
*/
t_ab8500_codec_error u8500_acodec_toggle_playback_mute_control(t_ab8500_codec_dest dest_device,t_u8500_bool_state mute_state,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	g_codec_system_context.output_config[dest_device].mute_state = mute_state;

	if(ENABLE == mute_state)
	{
		AB8500_CODEC_DestPowerControl(dest_device,AB8500_CODEC_SRC_STATE_ENABLE);
	}
	else
	{
		AB8500_CODEC_DestPowerControl(dest_device,AB8500_CODEC_SRC_STATE_DISABLE);
	}
...
* @out_right_volume - volume for right speaker
*/
t_ab8500_codec_error u8500_acodec_toggle_capture_mute_control(t_ab8500_codec_src src_device,t_u8500_bool_state mute_state,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	if(ENABLE == mute_state)
	{
		AB8500_CODEC_SrcPowerControl(src_device,AB8500_CODEC_SRC_STATE_ENABLE);
	}
	else
	{
		AB8500_CODEC_SrcPowerControl(src_device,AB8500_CODEC_SRC_STATE_DISABLE);
	}
...
t_ab8500_codec_error u8500_acodec_allocate_ad_slot(t_ab8500_codec_src input_device,t_u8500_mode mode)
{
	t_ab8500_codec_cr31_to_cr46_ad_data_allocation ad_data_line1,ad_data_line2;
	...
	ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_UNDEFINED;
	ad_data_line2 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_UNDEFINED;
	...
	switch(input_device)
	{
			case AB8500_CODEC_SRC_D_MICROPHONE_1:
			{
				...
				ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT1;
			}
			break;
			...
			case AB8500_CODEC_SRC_D_MICROPHONE_2:
			{
				...
				ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT2;
			}
			break;
			...
			case AB8500_CODEC_SRC_MICROPHONE_1B:
			{
				...
				ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT3;
			}
			break;
			case AB8500_CODEC_SRC_D_MICROPHONE_4:
			{
				slot1 = AB8500_CODEC_SLOT3;
				ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT4;
			}
			break;
			case AB8500_CODEC_SRC_D_MICROPHONE_5:
			{
				slot1 = AB8500_CODEC_SLOT4;
				ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT5;
			}
			break;
			case AB8500_CODEC_SRC_D_MICROPHONE_6:
			{
				slot1 = AB8500_CODEC_SLOT5;
				ad_data_line1 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT6;
			}
			break;
			case AB8500_CODEC_SRC_LINEIN:
			{
				...
				ad_data_line2 = AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_OUT2;
			}
			break;
	...
	if((AB8500_CODEC_SLOT_UNDEFINED != slot1) && (AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_UNDEFINED != ad_data_line1))
	{
		if(CLASSICAL_MODE == mode)
		{
				slot1 = AB8500_CODEC_SLOT0;
		}
		codec_error = AB8500_CODEC_ADSlotAllocation(slot1,ad_data_line1);
		if (AB8500_CODEC_OK != codec_error)
		{
			stm_error("ab8500_codec_adslot_allocation failed\n");
			return AB8500_CODEC_ERROR;
	...
	if((AB8500_CODEC_SLOT_UNDEFINED != slot2)&& (AB8500_CODEC_CR31_TO_CR46_SLOT_OUTPUTS_DATA_FROM_AD_UNDEFINED != ad_data_line2))
	{
		if(CLASSICAL_MODE == mode)
		{
				slot2 = AB8500_CODEC_SLOT1;
		}
		codec_error = AB8500_CODEC_ADSlotAllocation(slot2,ad_data_line2);
		if (AB8500_CODEC_OK != codec_error)
		{
			stm_error("ab8500_codec_adslot_allocation failed\n");
			return AB8500_CODEC_ERROR;
...
t_ab8500_codec_error u8500_acodec_unallocate_ad_slot(t_ab8500_codec_src input_device,t_u8500_mode mode)
{
	t_ab8500_codec_slot slot1,slot2;
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	if(AB8500_CODEC_SLOT_UNDEFINED != slot1)
	{
		...
		codec_error = AB8500_CODEC_ADSlotAllocation(slot1,AB8500_CODEC_CR31_TO_CR46_SLOT_IS_TRISTATE);
		if (AB8500_CODEC_OK != codec_error)
		{
			stm_error("ab8500_codec_adslot_allocation failed\n");
			return AB8500_CODEC_ERROR;
	...
	if(AB8500_CODEC_SLOT_UNDEFINED != slot2)
	{
		...
		codec_error = AB8500_CODEC_ADSlotAllocation(slot2,AB8500_CODEC_CR31_TO_CR46_SLOT_IS_TRISTATE);
		if (AB8500_CODEC_OK != codec_error)
		{
			stm_error("ab8500_codec_adslot_allocation failed\n");
			return AB8500_CODEC_ERROR;
...
t_ab8500_codec_error u8500_acodec_allocate_da_slot(t_ab8500_codec_dest output_device,t_u8500_mode mode)
{
	...
	if((AB8500_CODEC_DA_CHANNEL_NUMBER_UNDEFINED!=da_ch_no1) && (AB8500_CODEC_CR51_TO_CR56_SLTODA_SLOT_UNDEFINED!= da_slot1))
	{
		if(CLASSICAL_MODE == mode)
		{
				da_slot1 = AB8500_CODEC_CR51_TO_CR56_SLTODA_SLOT08;
		}
		codec_error = AB8500_CODEC_DASlotAllocation (da_ch_no1,da_slot1);
		if (AB8500_CODEC_OK != codec_error) {
			stm_error
			    ("ab8500_codec_daslot_allocation failed\n");
			return AB8500_CODEC_ERROR;
	...
	if((AB8500_CODEC_DA_CHANNEL_NUMBER_UNDEFINED!=da_ch_no2) && (AB8500_CODEC_CR51_TO_CR56_SLTODA_SLOT_UNDEFINED!= da_slot2))
	{
		if(CLASSICAL_MODE == mode)
		{
				da_slot1 = AB8500_CODEC_CR51_TO_CR56_SLTODA_SLOT09;
		}
		codec_error = AB8500_CODEC_DASlotAllocation (da_ch_no2,da_slot2);
		if (AB8500_CODEC_OK != codec_error) {
			stm_error
			    ("ab8500_codec_daslot_allocation failed\n");
			return AB8500_CODEC_ERROR;
...
t_ab8500_codec_error u8500_acodec_unallocate_da_slot(t_ab8500_codec_dest output_device,t_u8500_mode mode)
{
	return AB8500_CODEC_OK;
}


t_ab8500_codec_error u8500_acodec_set_src_power_cntrl(t_ab8500_codec_src input_device,
																		t_u8500_bool_state pwr_state)
{
	...
	if(ENABLE == pwr_state)
	{
		...
		codec_error = AB8500_CODEC_SrcPowerControl(input_device, AB8500_CODEC_SRC_STATE_ENABLE);
		if (AB8500_CODEC_OK != codec_error) {
			stm_error
			    ("AB8500_CODEC_SrcPowerControl failed\n");
			return AB8500_CODEC_ERROR;
		}
		g_codec_system_context.input_config[input_device].power_state = ENABLE;
	}
	else
	{
		u8500_acodec_unallocate_ad_slot(input_device,TDM_8_CH_MODE);
		codec_error = AB8500_CODEC_SrcPowerControl(input_device, AB8500_CODEC_SRC_STATE_DISABLE);
		if (AB8500_CODEC_OK != codec_error) {
			stm_error
			    ("AB8500_CODEC_SrcPowerControl failed\n");
			return AB8500_CODEC_ERROR;
		}
		g_codec_system_context.input_config[input_device].power_state = DISABLE;
	}

	return AB8500_CODEC_OK;
}

t_u8500_bool_state u8500_acodec_get_src_power_state(t_ab8500_codec_src input_device)
{
	return (g_codec_system_context.input_config[input_device].power_state);
}


t_ab8500_codec_error u8500_acodec_set_dest_power_cntrl(t_ab8500_codec_dest output_device,
																		t_u8500_bool_state pwr_state)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	if(ENABLE == pwr_state)
	{
		...
		codec_error = AB8500_CODEC_DestPowerControl(output_device,AB8500_CODEC_DEST_STATE_ENABLE);
		if (AB8500_CODEC_OK != codec_error) {
			stm_error
			    ("AB8500_CODEC_DestPowerControl failed\n");
			return AB8500_CODEC_ERROR;
		}
		g_codec_system_context.output_config[output_device].power_state = ENABLE;
	}
	else
	{
		u8500_acodec_unallocate_da_slot(output_device,TDM_8_CH_MODE);
		codec_error = AB8500_CODEC_DestPowerControl(output_device,AB8500_CODEC_DEST_STATE_DISABLE);
		if (AB8500_CODEC_OK != codec_error) {
			stm_error
			    ("AB8500_CODEC_DestPowerControl failed\n");
			return AB8500_CODEC_ERROR;
		}
		g_codec_system_context.output_config[output_device].power_state = DISABLE;
	}

	return AB8500_CODEC_OK;
}

t_u8500_bool_state u8500_acodec_get_dest_power_state(t_ab8500_codec_dest output_device)
{
	return (g_codec_system_context.output_config[output_device].power_state);
}
...
* This routine selects the output device Headphone or loud speaker
*/
t_ab8500_codec_error u8500_acodec_toggle_analog_lpbk(t_u8500_bool_state lpbk_state,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	if(ENABLE == lpbk_state)
	{
		...
		codec_error = AB8500_CODEC_SrcPowerControl(AB8500_CODEC_SRC_LINEIN,AB8500_CODEC_SRC_STATE_ENABLE);

		//codec_error = AB8500_CODEC_SetSrcVolume(AB8500_CODEC_SRC_LINEIN,VOL_MAX,VOL_MAX);

		codec_error = AB8500_CODEC_SelectOutput(AB8500_CODEC_DEST_HEADSET);

		codec_error = AB8500_CODEC_DestPowerControl(AB8500_CODEC_DEST_HEADSET,AB8500_CODEC_DEST_STATE_ENABLE);
...
//		ret_val = u8500_msp_receive_data(alsa_msp_adev,p_cnxt->buffer[p_cnxt->rx_index],p_cnxt->data_size);
		...
		#endif
		u8500_acodec_receive_data((void *)sample, count, 0);

		#if DRIVER_DEBUG > 1
		stm_dbg(DBG_ST.alsa," Transmitting \n");
		#endif
		u8500_acodec_send_data((void*)sample,count,0);
...
static void u8500_digital_lpbk_tx_dma_start()
{
	int index  = digital_lpbk_cnxt.tx_index;

	u8500_acodec_send_data((void*)digital_lpbk_cnxt.buffer[index].addr,digital_lpbk_cnxt.data_size,1);

	stm_dbg(DBG_ST.alsa,"Tx DMA Transfer started\n");
	stm_dbg(DBG_ST.alsa,"Tx: add = %x  index = %d size=%d\n", (int)(digital_lpbk_cnxt.buffer[index].addr),index, digital_lpbk_cnxt.data_size);

	digital_lpbk_cnxt.tx_index++;
	if(digital_lpbk_cnxt.tx_index >= DIGITAL_LPBK_MAX_BIFFERS)
	{
		digital_lpbk_cnxt.tx_index = 0;
	}
}

static void u8500_digital_lpbk_rx_dma_start()
{
	int index = digital_lpbk_cnxt.rx_index;

	u8500_acodec_receive_data((void *)digital_lpbk_cnxt.buffer[index].addr, digital_lpbk_cnxt.data_size, 1);

	stm_dbg(DBG_ST.alsa," Rx DMA Transfer started\n");
	stm_dbg(DBG_ST.alsa," Rx : add = %x  index = %d size=%d\n", (int)(digital_lpbk_cnxt.buffer[index].addr),index, digital_lpbk_cnxt.data_size);

	digital_lpbk_cnxt.rx_index++;
	if(digital_lpbk_cnxt.rx_index >= DIGITAL_LPBK_MAX_BIFFERS)
	{
		digital_lpbk_cnxt.rx_index = 0;
	}
}

static irqreturn_t tx_rx_dma_eot_handler(void *data, int irq)
{
	int *stream = data;

	if(digital_lpbk_cnxt.tx_active == DISABLE)
	{
		return IRQ_HANDLED;
	}
	...
	if(play_flag == *stream)
	{
		stm_dbg(DBG_ST.alsa,"Rx: One transfer complete.. starting next\n");
		u8500_digital_lpbk_tx_dma_start();
	}
	else if(capture_flag == *stream)
	{
		stm_dbg(DBG_ST.alsa,"Tx: One transfer complete.. starting next\n");
		u8500_digital_lpbk_rx_dma_start();
	}
	return IRQ_HANDLED;
}
...
* This routine selects the output device Headphone or loud speaker
*/
t_ab8500_codec_error u8500_acodec_toggle_digital_lpbk(t_u8500_bool_state lpbk_state,t_acodec_user user)
{
	t_ab8500_codec_error codec_error = AB8500_CODEC_OK;
	...
	stm_dbg(DBG_ST.acodec,"  Entering u8500_acodec_toggle_digital_lpbk() \n");
	...
	if(ENABLE == lpbk_state)
	{
		...
		status = u8500_acodec_open(0);
		if(status)
		{
			printk("failed in getting acodec playback open\n");
			return -1;
		}
		status = u8500_acodec_open(1);
		if(status)
		{
	...
	acodec_config.handler=tx_rx_dma_eot_handler;
	acodec_config.tx_callback_data= (void *)&play_flag;
	acodec_config.rx_callback_data= (void *)&capture_flag;
#endif
	u8500_acodec_enable_audio_mode(&acodec_config);

	//	address = (ab8500_AUDIO<<8)|0x1c;
		//ab8500_write(ab8500_AUDIO,address,0x14);

		//codec_error = AB8500_CODEC_SelectInput(AB8500_CODEC_SRC_D_MICROPHONE_3);

		u8500_acodec_set_src_power_cntrl(AB8500_CODEC_SRC_D_MICROPHONE_1,ENABLE);
		u8500_acodec_set_input_volume(AB8500_CODEC_SRC_D_MICROPHONE_1,50,50,USER_ALSA);
		...
		//codec_error = AB8500_CODEC_SrcPowerControl(AB8500_CODEC_SRC_D_MICROPHONE_1,AB8500_CODEC_SRC_STATE_ENABLE);
		//codec_error = AB8500_CODEC_SetSrcVolume(AB8500_CODEC_SRC_D_MICROPHONE_1,50,50);

		//codec_error = AB8500_CODEC_SrcPowerControl(AB8500_CODEC_SRC_D_MICROPHONE_2,AB8500_CODEC_SRC_STATE_ENABLE);
		//codec_error = AB8500_CODEC_SetSrcVolume(AB8500_CODEC_SRC_D_MICROPHONE_2,50,50);

		//u8500_acodec_allocate_ad_slot(AB8500_CODEC_SRC_LINEIN,TDM_8_CH_MODE);
		//codec_error = AB8500_CODEC_SrcPowerControl(AB8500_CODEC_SRC_LINEIN,AB8500_CODEC_SRC_STATE_ENABLE);
		//codec_error = AB8500_CODEC_SetSrcVolume(AB8500_CODEC_SRC_LINEIN,70,70);
		...
		//codec_error = AB8500_CODEC_SetDestVolume(AB8500_CODEC_DEST_HEADSET,100,100);

		digital_lpbk_cnxt.data_size = 1024;
		...
		for(i=0;i<DIGITAL_LPBK_MAX_BIFFERS;i++)
		{
			digital_lpbk_cnxt.buffer[i].area = dma_alloc_coherent(NULL, digital_lpbk_cnxt.data_size, &digital_lpbk_cnxt.buffer[i].addr, GFP_KERNEL);
			if(NULL == digital_lpbk_cnxt.buffer[i].area)
			{
	...
	{
		pid_t pid_rx_tx;
		pid_rx_tx = kernel_thread(digital_lpbk_msp_rx_tx_thread, &digital_lpbk_cnxt, CLONE_FS | CLONE_SIGHAND);
	}
#elif defined(CONFIG_U8500_ACODEC_DMA)
	{
		u8500_digital_lpbk_rx_dma_start();
	...
	else //lpbk is disable
	{
		...
		for(i=0;i<DIGITAL_LPBK_MAX_BIFFERS;i++)
		{
			dma_free_coherent(NULL, digital_lpbk_cnxt.data_size,digital_lpbk_cnxt.buffer[i].area, digital_lpbk_cnxt.buffer[i].addr);
		}

		u8500_acodec_set_src_power_cntrl(AB8500_CODEC_SRC_D_MICROPHONE_1,DISABLE);
		u8500_acodec_set_src_power_cntrl(AB8500_CODEC_SRC_D_MICROPHONE_2,DISABLE);
		u8500_acodec_set_src_power_cntrl(AB8500_CODEC_SRC_D_MICROPHONE_3,DISABLE);
		u8500_acodec_set_src_power_cntrl(AB8500_CODEC_SRC_D_MICROPHONE_4,DISABLE);

		u8500_acodec_set_dest_power_cntrl(AB8500_CODEC_DEST_HEADSET,DISABLE);
		u8500_acodec_set_dest_power_cntrl(AB8500_CODEC_DEST_HANDSFREE,DISABLE);
...
t_ab8500_codec_error u8500_acodec_set_burst_mode_fifo(t_u8500_pmc_rendering_state fifo_state)
{
	...
	if(RENDERING_ENABLE == fifo_state)
	{
		...
		burst_fifo_config.cr106_bfifofsext  = AB8500_CODEC_CR106_BFIFOFSEXT_6SLOT_EXTRA_CLK;
		burst_fifo_config.cr106_bfifomsk    = AB8500_CODEC_CR106_BFIFOMSK_AD_DATA0_UNMASKED;
		burst_fifo_config.cr106_bfifomstr   = AB8500_CODEC_CR106_BFIFOMSTR_MASTER_MODE;
		burst_fifo_config.cr106_bfifostrt   = AB8500_CODEC_CR106_BFIFOSTRT_RUNNING;
		burst_fifo_config.cr107_bfifosampnr = 0x100;
		burst_fifo_config.cr108_bfifowakeup = 0x1;

		ab8500_codec_error = AB8500_CODEC_ConfigureBurstFifo (&burst_fifo_config);
...
t_ab8500_codec_error dump_msp_registers()
{
		...
		for (i = 0; i < 0x40; i += 4)
			stm_dbg(DBG_ST.acodec,"msp[0x%x]=0x%x\n", i,readl((char *)(IO_ADDRESS(U8500_MSP1_BASE) +i)));
...
static int i2sdrv_probe(struct i2s_device *i2s)
{
	...
	i2sdrv.tx_status = 0;
	i2sdrv.rx_status = 0;
        spin_lock_init(&i2sdrv.i2s_lock);
	try_module_get(i2s->controller->dev.parent->driver->owner);
	i2s_set_drvdata(i2s, (void*)&i2sdrv);
...
static void ab8500_codec_power_init(void)
{
	...
	old_data = ab8500_read(AB8500_SYS_CTRL2_BLOCK,(AB8500_CTRL3_REG & 0xFF));
	data = 0x02|old_data;
              ab8500_write(AB8500_SYS_CTRL2_BLOCK,(AB8500_CTRL3_REG & 0xFF),data);//0x0200


    old_data = ab8500_read(AB8500_SYS_CTRL2_BLOCK,(AB8500_SYSULPCLK_CTRL1_REG  & 0xFF));
    data = 0x10|old_data;
              ab8500_write(AB8500_SYS_CTRL2_BLOCK,(AB8500_SYSULPCLK_CTRL1_REG & 0xFF),data);//0x020B


	old_data = ab8500_read(AB8500_REGU_CTRL1,(AB8500_REGU_MISC1_REG & 0xFF));
    data = 0x04|old_data;
              ab8500_write(AB8500_REGU_CTRL1,(AB8500_REGU_MISC1_REG & 0xFF),data);


	old_data = ab8500_read(AB8500_REGU_CTRL1,(AB8500_REGU_VAUDIO_SUPPLY_REG & 0xFF));
    data = 0x5E|old_data;
              ab8500_write(AB8500_REGU_CTRL1,(AB8500_REGU_VAUDIO_SUPPLY_REG & 0xFF),data);
...
t_ab8500_codec_error AB8500_CODEC_Write(IN t_uint8 register_offset, IN t_uint8 count, IN t_uint8 *ptr_data)
{
	int i;
	u32 address;
...
t_ab8500_codec_error AB8500_CODEC_Read(IN t_uint8 register_offset, IN t_uint8 count, IN t_uint8 *dummy_data, IN t_uint8 *ptr_data)
{
	int i;
	u32 address;
